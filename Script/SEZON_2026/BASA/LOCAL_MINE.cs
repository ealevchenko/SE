...
/// <summary>
/// v1.0
/// Локальная шахта V1.21-09-2026
/// </summary>
namespace LOCAL_MINE
{
    public sealed class Program : MyGridProgram
    {
        ...
        public Program()
        {
            ...
            lcd_pistons = new LCD(NameObj + "-LCD-Pistons");
            bats = new Batterys(NameObj);
            //leds = new Lightings(NameObj, tag_led);
            pist = new Pistons(NameObj, tag_mine);
            upr = new Upr();
        ...
        public class Pistons : BaseListTerminalBlock<IMyExtendedPistonBase>
        {
            ...
            public Pistons(string name_obj, string tag) : base(name_obj) { if (!String.IsNullOrWhiteSpace(tag)) { list_obj = list_obj.Where(n => n.CustomName.Contains(tag)).ToList(); } _scr.Echo("Найдено PistonBase:[" + tag + "]: " + list_obj.Count()); }
            ...
            public bool SetPosition(float position, float speed)
            {
                double difference = position - this.Position;
                if (Math.Abs(difference) > tolerance)
                {
                    if (difference > 0) { SetVelocity(speed); return false; }
                    else if (difference < 0) { SetVelocity(-speed); return false; }
                    else { SetVelocity(0); return true; }
                }
                else
                { SetVelocity(0); return true; }
            }
            public bool SetPosition(float position)
            {
                ...
                if (Math.Abs(difference) > tolerance)
                {
                    if (difference != 0) { SetVelocity((float)(difference * multiply_speed)); return false; }
                    else { SetVelocity(0); return true; }
                }
                else { SetVelocity(0); return true; }
            }
