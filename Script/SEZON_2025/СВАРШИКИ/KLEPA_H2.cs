...
/// <summary>
///  СВАРЩИК ВОДОРОДНЫЙ (v1.0) - сезон 2025
/// </summary>
namespace KLEPA_H2
{
    public sealed class Program : MyGridProgram
    {
        ...
        public class Nav
        {
            ...
            public static string[] name_mode = { "", "БАЗА", "РАСТЫКОВКА", "К БАЗЕ", "СТЫКОВКА", "НА ТОЧКУ БАЗЫ" };
            ...
            public Vector3D BaseDockPoint { get; set; } = new Vector3D(0, safe_base_height, -safe_base_distance);
            ...
            public bool Dock()
            {
                ...
                if (ForwVelocityVector.Length() < MaxFSpeed && MyPosCon.GetDim(2) < 0 && (
                    (HDistance >= safe_base_distance) ||
                    (Math.Abs(MyPosCon.GetDim(2)) < safe_base_distance && Math.Abs(MyPosCon.GetDim(0)) < 1.0f && Math.Abs(MyPosCon.GetDim(1)) < 1.0f)))
                {
            ...
            public bool ToPointBase()
            {
                ...
                HDistance = (float)(new Vector3D(0, 0, -safe_base_distance) - new Vector3D(MyPosCon.GetDim(0), 0, MyPosCon.GetDim(2))).Length();
                VDistance = (float)(new Vector3D(0, safe_base_height, 0) - new Vector3D(0, MyPosCon.GetDim(1), 0)).Length();
                gyros.SetOverride(BlockNav, true, gyrAng * GyroMult, 1);
                //MaxLSpeed = (float)Math.Sqrt(2 * Math.Abs(MyPosCon.GetDim(0)) * XMaxA) / 2; // 
                MaxUSpeed = (float)Math.Sqrt(2 * Math.Abs(MyPosCon.GetDim(1)) * YMaxA) / 2; //
                ...
                {
                    // в близи базы (safe_base_height)
                    if (Math.Abs(MyPosCon.GetDim(1) - safe_base_height) < 5.0f) MaxUSpeed = MinUDSpeed;
                    float UpAccel = getAccel(-(float)((MyPosCon.GetDim(1) - safe_base_height) * AlignAccelMult), MinUDSpeed);
                    ...
                    if (ForwVelocityVector.Length() < MaxFSpeed && Math.Abs(MyPosCon.GetDim(1) - safe_base_height) < 5.0f)
                    {
                        thrusts.SetOverrideAccel("F", (float)(HDistance * AlignAccelMult));
                        thrusts.SetOverridePercent("B", 0);
                    }
            ...
            public void Logic(string argument, UpdateType updateSource)
            {
                switch (argument)
                {
                    ...
                    case "to_point_base": { InitMode(mode.to_point_base); break; }
