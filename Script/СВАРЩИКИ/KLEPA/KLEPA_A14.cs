...
{
    /// <summary>
    /// Сварщик (атмасферный) на 14 сварщиков
    /// </summary>
    public sealed class Program : MyGridProgram
    {
        // v3
        string NameObj = "[KLEPA-A14-01]";
        ...
        public Program()
        {
            ...
            lcd_info1 = new LCD(NameObj + "-LCD-INFO1");
            lcd_info2 = new LCD(NameObj + "-LCD-INFO2");
            lcd_info3 = new LCD(NameObj + "-LCD-INFO3");
            ...
            connector_up = new Connector(NameObj + "-Коннектор [up]");
        ...
        public void Main(string argument, UpdateType updateSource)
        {
            ...
            if (updateSource == UpdateType.Update10)
            {
                values_info.Append(bats.TextInfo("БАТ-ОСН"));
                values_info.Append(bats_duty.TextInfo("БАТ-ОСН"));
                values_info.Append(connector_back.TextInfo("К:Back") + " "+ connector_up.TextInfo("К:Up") + " " + landing_gears.TextInfo());
                ...
                if (clock_main >= 10)
                {
                    ...
                    lcd_info3.OutText(values_info2);
                }
                clock_main++;
            }
        }
        ...
        public class Navigation
        {
            ...
            public bool Dock()
            {
                ...
                {
                    if (connector_back.Status == MyShipConnectorStatus.Connectable)
                    {
                        connector_back.obj.Connect();
                    }
                    if (connector_back.Status == MyShipConnectorStatus.Connected)
                    {
                        thrusts.ClearThrustOverridePersent();
                        gyros.SetOverride(false, 1);
                        curent_mode = mode.none;
            ...
            public void Logic(string argument, UpdateType updateSource)
            {
                switch (argument)
                {
                    ...
                    case "fly_base": curent_programm = programm.fly_connect_base; mystorage.SaveToStorage(); break;
                    case "fly_work": curent_programm = programm.fly_place_work; mystorage.SaveToStorage(); break;
                    ...
                    case "to_work": curent_mode = mode.to_work; mystorage.SaveToStorage(); break;
                    case "align_work": curent_mode = mode.align_work; mystorage.SaveToStorage(); break;
                    default: break;
                }
                if (updateSource == UpdateType.Update10)
                {
