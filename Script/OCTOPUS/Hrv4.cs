...
namespace Hrv4
{
    public sealed class Program : MyGridProgram
    {
        ...
        string NewName = "Hrv4";
        float GyroMult = 0.5f;
        int CriticalMass = 150000;
        float AlignAccelMult = 0.3f;
        float DrillAccel = 0.7f;
        float DrillSpeedLimit = 0.5f;
        float DrillGyroMult = 2f;
        float ReturnOnCharge = 0.2f;
        float DrillFrameWidth = 5f;
        float DrillFrameLength = 5f;
        float DrillDepth = 150;
        int MaxShafts = 50;
        ...
        public class MyDriller
        {
            ...
            private class MyThrusters
            {
                ...
                private static string Prefix = "Thrust";
                ...
                public void Update()
                {
                    ...
                    YMaxA = Math.Min(UMaxT, DMaxT) / TotalMass;
            ...
            private class MyNavigation
            {
                ...
                private IMyRemoteControl RemConRear;
                ...
                private List<MyDetectedEntityInfo> DetectedList = new List<MyDetectedEntityInfo>();
                ...
                public MyNavigation(MyDriller mdr)
                {
                    ...
                    RemConRear = ParentProgram.GridTerminalSystem.GetBlockWithName(myDriller.MyName + "RemConRear") as IMyRemoteControl;
                    Sensor = ParentProgram.GridTerminalSystem.GetBlockWithName(myDriller.MyName + "SensorFront") as IMySensorBlock;
                ...
                {
                    DockMatrix = GetTransMatrixFromMyPos();
                }

                public void SetDrillMatrixDepo()
                {
                    DrillMatrix = GetTransMatrixFromMyPos();
                    ShaftN = 0;
                    DrillPoint = new Vector3D(0, 0, 0);
                }
                public Vector3D GetNavAngles(Vector3D Target, MatrixD InvMatrix, double shiftX = 0, double shiftY = 0)
                {
                    ...
                    Vector3D TargetNorm = Vector3D.Normalize(Target - V3Dcenter) - new Vector3D(shiftX, shiftY, 0);
                    ...
                    TargetRoll = Vector3D.Dot(V3Dleft, Vector3D.Normalize(Vector3D.Reject(new Vector3D(0, 1, 0), V3Dfow)));
                ...
                public bool Dock()
                {
                    ...
                    {
                        myDriller.thrustBlock.SetOverrideAccel("B", (float)(Distance * ParentProgram.AlignAccelMult));
                ...
                public bool UnDock()
                {
                    ...
                    if (myDriller.connectorBlock.UnLock())
                    {
                        ...
                        myDriller.thrustBlock.SetOverridePercent("R", 0);
                        myDriller.thrustBlock.SetOverridePercent("L", 0);
                        myDriller.thrustBlock.SetOverridePercent("F", 10);
                        myDriller.thrustBlock.SetOverrideAccel("B", 0);
                        if (Distance > 50)
                        {
                            Complete = true;
                        }
                ...
                public bool ToDrillPoint()
                {
                    ...
                    Vector3D drTarget = new Vector3D(0, 0, -200);
                    Vector3D gyrAng = GetNavAngles(DrillPoint + drTarget, DrillMatrix);
                    ...
                    if (RemCon.GetNaturalGravity().LengthSquared() > 0.2f)
                        MaxUSpeed = (float)Math.Sqrt(2 * Math.Abs(FHTemp - (MyPos - PlanetCenter).Length()) * myDriller.thrustBlock.YMaxA) / 1.2f;
                    float Distance = (float)(DrillPoint + drTarget - MyPosDrill).Length();
                ...
                public bool DrillAlign()
                {
                    ...
                    if (UpVelocityVector.Length() < MaxUSpeed)
                        myDriller.thrustBlock.SetOverrideAccel("D", (float)(MyPosDrill.GetDim(1) * ParentProgram.AlignAccelMult));
                    else
                    {
                        myDriller.thrustBlock.SetOverridePercent("U", 0);
                        myDriller.thrustBlock.SetOverridePercent("D", 0);
                    }
                    if (ForwVelocityVector.Length() < MaxFSpeed)
                    {
                        float FAccel = (float)(MyPosDrill.GetDim(2) * ParentProgram.AlignAccelMult);
                        float minFAccel = 0.3f;
                        if ((FAccel < 0) && (FAccel > -minFAccel))
                            FAccel = -minFAccel;
                        if ((FAccel > 0) && (FAccel < minFAccel))
                            FAccel = minFAccel;
                        myDriller.thrustBlock.SetOverrideAccel("B", FAccel);
                    }
                    else
                    {
                        myDriller.thrustBlock.SetOverridePercent("B", 0);
                    ...
                    strStatus += "DrillPoint: " + DrillPoint + "\n";
                    strStatus += "Ship Mass: " + myDriller.thrustBlock.TotalMass.ToString() + "\n";
                    strStatus += "Battery charge: " + Math.Round(myDriller.batteryBlock.StoredPower * 100 / myDriller.batteryBlock.MaxPower, 2).ToString() + " % \n";
                    strStatus += "Z shift: " + Math.Round(MyPosDrill.GetDim(2), 2).ToString() + "\n";
                    strStatus += "XY shifts: " + Math.Round(MyPosDrill.GetDim(0), 2).ToString() + " / " + Math.Round(MyPosDrill.GetDim(1), 2).ToString() + "\n";
                    strStatus += "Shafts drilled: " + ShaftN.ToString() + " / " + ParentProgram.MaxShafts.ToString() + "\n";
                    myDriller.TextOutput("TP_Status", strStatus);
                ...
                public bool Drill(out bool Emergency)
                {
                    ...
                    float MaxLSpeed, MaxUSpeed;
                    Vector3D MyPosDrill = Vector3D.Transform(MyPos, DrillMatrix) - DrillPoint;

                    double shiftX = MyPosDrill.GetDim(0) / 2;
                    if (shiftX < 0) shiftX = Math.Max(shiftX, -0.1);
                    else shiftX = Math.Min(shiftX, 0.1);
                    double shiftY = MyPosDrill.GetDim(1) / 2;
                    if (shiftY < 0) shiftY = Math.Max(shiftY, -0.1);
                    else shiftY = Math.Min(shiftY, 0.1);

                    Vector3D gyrAng = GetNavAngles(MyPosDrill + DrillPoint + new Vector3D(0, 0, 1), DrillMatrix, shiftX, shiftY);
                    ...
                    MaxUSpeed = (float)Math.Sqrt(2 * Math.Abs(MyPosDrill.GetDim(1)) * myDriller.thrustBlock.YMaxA) / 5;
                    ...
                    if (UpVelocityVector.Length() < MaxUSpeed)
                        myDriller.thrustBlock.SetOverrideAccel("D", (float)(MyPosDrill.GetDim(1) * 10));
                    else
                    {
                        myDriller.thrustBlock.SetOverridePercent("U", 0);
                        myDriller.thrustBlock.SetOverridePercent("D", 0);
                    ...
                    if ((ForwVelocityVector.Length() < ParentProgram.DrillSpeedLimit) && (!myDriller.cargoBlock.StoneDumpNeeded))
                    {
                        if ((Math.Abs(MyPosDrill.GetDim(0)) < 1) && (Math.Abs(MyPosDrill.GetDim(1)) < 1))
                            myDriller.thrustBlock.SetOverrideAccel("F", (ParentProgram.DrillAccel));
                        else
                        {
                            myDriller.thrustBlock.SetOverrideAccel("B", (ParentProgram.DrillAccel));
                            PullUpNeeded = true;
                            Complete = true;
                        }
                    }
                    ...
                    Sensor.DetectedEntities(DetectedList);
                    if ((MyPosDrill.GetDim(2) > ((myDriller.Command == Commands.DepoDrill) ? (ParentProgram.DrillDepth) : 16)) || DetectedList.Count == 0)
                        Complete = true;
                    ...
                    strStatus += "Ship Mass: " + myDriller.thrustBlock.TotalMass.ToString() + "\n";
                    strStatus += "Battery charge: " + Math.Round(myDriller.batteryBlock.StoredPower * 100 / myDriller.batteryBlock.MaxPower, 2).ToString() + " % \n";
                    strStatus += "Depth: " + Math.Round(MyPosDrill.GetDim(2), 2).ToString() + " / " + Math.Round(((myDriller.Command == Commands.DepoDrill) ? (ParentProgram.DrillDepth) : 16), 2).ToString() + "\n";
                    strStatus += "XY shifts: " + Math.Round(MyPosDrill.GetDim(0), 2).ToString() + " / " + Math.Round(MyPosDrill.GetDim(1), 2).ToString() + "\n";
                    strStatus += "Shafts drilled: " + ShaftN.ToString() + " / " + ParentProgram.MaxShafts.ToString() + "\n";
                    myDriller.TextOutput("TP_Status", strStatus);
                ...
                public bool PullUp()
                {
                    ...
                    if (ForwVelocityVector.Length() < ParentProgram.DrillSpeedLimit * 5)
                        myDriller.thrustBlock.SetOverrideAccel("B", (ParentProgram.DrillAccel * 2));
                    else
                    {
                        myDriller.thrustBlock.SetOverridePercent("F", 0);
                    }

                    if ((MyPosDrill.GetDim(2) < 0) || ((Math.Abs(MyPosDrill.GetDim(0)) < 0.15) && (Math.Abs(MyPosDrill.GetDim(1)) < 0.15)))
                    {
                        Complete = true;
                        PullUpNeeded = false;
                    }
                ...
                public bool PullOut()
                {
                    ...
                    if (ForwVelocityVector.Length() < ParentProgram.DrillSpeedLimit * 5 && Math.Abs(MyPosDrill.GetDim(0)) < 0.5 && Math.Abs(MyPosDrill.GetDim(1)) < 0.5)
                        myDriller.thrustBlock.SetOverrideAccel("B", (ParentProgram.DrillAccel * 2));
                    else
                    {
                        myDriller.thrustBlock.SetOverridePercent("B", 0);
                    ...
                    {
                        if (!(myDriller.EmergencyReturn || myDriller.GoHome) || (MyPosDrill.GetDim(2) < -200))
                        {
                            Complete = true;
                            myDriller.thrustBlock.SetOverrideAccel("B", 0f);
                        }
                        else
                        {
                            myDriller.thrustBlock.SetOverridePercent("B", 100f);
                        }
                    }
                    ...
                    strStatus += "Battery charge: " + Math.Round(myDriller.batteryBlock.StoredPower * 100 / myDriller.batteryBlock.MaxPower, 2).ToString() + " % \n";
                    strStatus += "Depth: " + Math.Round(-MyPosDrill.GetDim(2), 2).ToString() + " / " + Math.Round(((myDriller.Command == Commands.DepoDrill) ? (ParentProgram.DrillDepth) : 16), 2).ToString() + "\n";
                    strStatus += "XZ shifts: " + Math.Round(MyPosDrill.GetDim(0), 2).ToString() + " / " + Math.Round(MyPosDrill.GetDim(1), 2).ToString() + "\n";
                    strStatus += "Shafts drilled: " + ShaftN.ToString() + " / " + ParentProgram.MaxShafts.ToString() + "\n";
                    myDriller.TextOutput("TP_Status", strStatus);
            ...
            private class MyDrills
            {
                ...
                public void Turn(string OnOff)
                {
                    for (int i = 0; i < DrillGroup.Count; i++)
                    {
                        ...
                        if (drill != null)
                            drill.ApplyAction("OnOff_" + OnOff);
                    }
                    if (OnOff == "On")
                        DrillSOn = true;
                    else
