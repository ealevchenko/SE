...
{
    /// <summary>
    /// Транспортный корабль, орбитальный перевозчик груза, буровиков и топлива (поиск руды на метиоритах)
    /// </summary>
    public sealed class Program : MyGridProgram
    {
        // v4
        string NameObj = "[TRUCK-H1-01]";
        static float BaseDistance = 200f;
        static float dist_con_forw_cocpit = 20f;       // Растояние от коннектора стыковки к корпиту
        static float dist_con_back_cocpit = 20f;       // Растояние от коннектора стыковки к корпиту
        ...
        static Connector connector_l_ship;
        static Connector connector_r_ship;
        static ShipMergeBlock mergeblock_back1, mergeblock_back2; // 
        static Gyros gyros;
        ...
        public Program()
        {
            ...
            connector_l_ship = new Connector(NameObj + "-Коннектор l_ship");
            connector_r_ship = new Connector(NameObj + "-Коннектор r_ship");
            mergeblock_back1 = new ShipMergeBlock(NameObj + "-Соединитель груз1");
            mergeblock_back1.On();
            mergeblock_back2 = new ShipMergeBlock(NameObj + "-Соединитель груз2");
            mergeblock_back2.On();
        ...
        public void Main(string argument, UpdateType updateSource)
        {
            ...
            if (updateSource == UpdateType.Update10)
            {
                ...
                values_info.Append(hydrogen_tanks_nav.TextInfo("H2-ГРУЗ"));
                values_info.Append(connector_forw.TextInfo("К:Forw") + "\n");
                values_info.Append(connector_back.TextInfo("К:Back") + "\n");
                values_info.Append(connector_l_ship.TextInfo("К:l_ship") + " ");
                values_info.Append(connector_r_ship.TextInfo("К:r_ship") + "\n");
                values_info.Append(mergeblock_back1.TextInfo("СОЕД-НОСИТЕЛЬ-1"));
                values_info.Append(mergeblock_back1.TextInfo("СОЕД-НОСИТЕЛЬ-2"));
                ...
                values_lcd_info.Append(hydrogen_tanks_nav.TextInfo("H2-ГРУЗ"));
                values_lcd_info.Append(connector_forw.TextInfo("К:Forw") + "\n");
                values_lcd_info.Append(connector_back.TextInfo("К:Back") + "\n");
                values_lcd_info.Append(connector_l_ship.TextInfo("К:l_ship") + " ");
                values_lcd_info.Append(connector_r_ship.TextInfo("К:r_ship") + "\n");
                values_lcd_info.Append(mergeblock_back1.TextInfo("СОЕД-НОСИТЕЛЬ-1"));
                values_lcd_info.Append(mergeblock_back1.TextInfo("СОЕД-НОСИТЕЛЬ-2"));
        ...
        public class Navigation
        {
            ...
            public void UpdateCalc()
            {
                ...
                CriticalBatteryCharge = connector_forw.Connected || connector_back.Connected ? bats.CurrentPersent() < 1.0f : bats.CurrentPersent() <= CriticalOnCharge;
                CriticalHydrogenSupply = connector_forw.Connected || connector_back.Connected ? hydrogen_tanks_nav.AverageFilledRatio < 1.0f : hydrogen_tanks_nav.AverageFilledRatio <= CriticalOnH2;
            ...
            public void Logic(string argument, UpdateType updateSource)
            {
                ...
                if (updateSource == UpdateType.Update10)
                {
                    ...
                    {
                        if (gravity || (!gravity && connector_forw.Connectable))
                        {
                            bats.Auto();
                            thrusts.On();
                        }
