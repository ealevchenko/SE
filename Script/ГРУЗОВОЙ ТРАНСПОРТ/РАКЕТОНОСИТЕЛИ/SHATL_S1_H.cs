...
namespace SHATL_S1_H
{
    /// <summary>
    /// Ракетоноситель ступень 1 (с возвратом на планету), вывода на орбиту ступени 2 (v1.0)
    /// </summary>
    public sealed class Program : MyGridProgram
    {

        // v1
        string NameObj = "[SHATL-S1-01]";
        ...
        static float min_br_height = 1000f;     // минимальное растояние от земли, начинаем тормозить
        static float GyroMult = 10f;
        //static float DrillGyroMult = 10f;
        //static float AlignAccelFMult = 0.3f;
        static float AlignAccelMult = 0.9f;
        static float AlignFAccelMult = 0.3f;
        ...
        static Connector connector, connector_s2;
        ...
        public Program()
        {
            ...
            lcd_info1 = new LCD(NameObj + "-LCD-Info 1");
            lcd_info2 = new LCD(NameObj + "-LCD-Info 2");
            ...
            connector_s2 = new Connector(NameObj + "-Коннектор s2");
            mergeblock_cargo1 = new ShipMergeBlock(NameObj + "-Соединитель s2-1");
            mergeblock_cargo1.On();
            mergeblock_cargo2 = new ShipMergeBlock(NameObj + "-Соединитель s2-2");
        ...
        public class Nav
        {
            ...
            public static string[] name_mode = { "", "БАЗА", "РАСТЫКОВКА", "К БАЗЕ", "СТЫКОВКА", "НА ОРБИТУ" };
            ...
            public float ForwThrust { get; private set; }
            //--------------------
            ...
            //public Vector3D ConnectorPoint { get; set; } = new Vector3D(0, 0, -100);
            public Vector3D ConnectorPoint { get; set; } = new Vector3D(0, -(int)(safe_base_distance / 2), 0);
            public Vector3D ConnectorPoint1 { get; set; } = new Vector3D(0, (int)(safe_base_distance / 2), 0);

            //public Vector3D BaseDockPoint { get; set; } = new Vector3D(0, 0, -safe_base_height);
            ...
            //public void InitMode(mode mode) { if (mode == mode.none || mode == mode.un_dock || mode == mode.dock || mode == mode.base_operation) InitBlock(connector.obj); else InitBlock(cockpit.obj); curent_mode = mode; strg.SaveToStorage(); }
            public void InitMode(mode mode) { if (mode != mode.none) InitBlock(connector.obj); else InitBlock(cockpit.obj); curent_mode = mode; strg.SaveToStorage(); }
            public float getAccel(float Accel, float minFAccel = 0.1f)
            {
                if ((Accel < 0) && (Accel > -minFAccel)) return -minFAccel; if ((Accel > 0) && (Accel < minFAccel)) return minFAccel; return Accel;
            }
            public Vector3D GetNavAngles(Vector3D Target, MatrixD InvMatrix)
            {
                ...
                if (gravity)
                {
                    ...
                    GravNorm = Vector3D.Normalize(Vector3D.Transform(GravNorm + MyPos, InvMatrix) - V3Dcenter);
                }
                //Получаем проекции вектора прицеливания на все три оси блока ДУ. 
            ...
            public Vector3D GetForwNavAngles(Vector3D Target, MatrixD InvMatrix)
            {
                Vector3D V3Dcenter = MyPos;
                Vector3D V3Dfow = WMCocpit.Forward + V3Dcenter;
                Vector3D V3Dup = WMCocpit.Up + V3Dcenter;
                ...
                //double gL = GravNorm.Dot(V3Dleft);
                //double gU = GravNorm.Dot(V3Dup);
                double gF = GravNorm.Dot(V3Dup);
                double gL = GravNorm.Dot(-V3Dleft);
                double gU = GravNorm.Dot(-V3Dfow);
                //Получаем сигналы по тангажу и крены операцией atan2
                double TargetRoll = (float)Math.Atan2(gL, -gU); // крен
                double TargetPitch = -(float)Math.Atan2(gF, -gU); // тангаж
                Vector3D TargetNorm = Vector3D.Normalize(Target - V3Dcenter);
            ...
            public void UpdateCalc()
            {
                ...
                if (BlockNav != null)
                {
                    ...
                    ForwThrust = (float)(GravVector * PhysicalMass).Dot(WMCocpit.Forward);
                ...
                YMaxA = (float)Math.Min(thrusts.UpThrMax, thrusts.DownThrMax) / PhysicalMass;
                //ZMaxA = (float)Math.Min(thrusts.ForwardThrMax, thrusts.BackwardThrMax) / PhysicalMass;
                ZMaxA = Math.Abs((float)Math.Min(thrusts.ForwardThrMax / PhysicalMass - GravVector.Length(), thrusts.BackwardThrMax / PhysicalMass + GravVector.Length()));
                XMaxA = (float)Math.Min(thrusts.RightThrMax, thrusts.LeftThrMax) / PhysicalMass;

                // расчтет пути торможения (посадка Down-внизу)
                double a = (thrusts.ForwardThrMax / 1000) * (1 / (PhysicalMass / 1000));
                double t = (0 - ForwVelocityVector.Length()) / -a; //t = (V - V[0]) / a
                DownBrDistance = (ForwVelocityVector.Length() * t) + ((-a) * Math.Pow(t, 2)) / 2; //S = V[0] * t + ( a * t^2 ) / 2
                ...
                values1.Append("ForwThrust  : " + Math.Round(ForwThrust, 2) + "\n");
                values1.Append("ForwVelocityVector  : " + Math.Round(ForwVelocityVector.Length(), 2) + "\n");
                values1.Append("LeftVelocityVector  : " + Math.Round(LeftVelocityVector.Length(), 2) + "\n");
                values1.Append("UpVelocityVector  : " + Math.Round(UpVelocityVector.Length(), 2) + "\n");
                ...
                values3.Append(thrusts.TextInfo());
                lcd_test3.OutText(values3);
            ...
            public bool UnDock()
            {
                ...
                if (!connector.Connected)
                {
                    ...
                    Vector3D gyrAng = GetForwNavAngles(ConnectorPoint, DockMatrix);
                    gyros.SetOverride(BlockNav, true, gyrAng * GyroMult, 1);
                    thrusts.SetOverridePercent("U", 0f);
                    thrusts.SetOverridePercent("D", 0f);
                    thrusts.SetOverridePercent("R", 0f);
                    thrusts.SetOverridePercent("L", 0f);
                    thrusts.SetOverridePercent("F", 0f);
                    thrusts.SetOverridePercent("B", 1.0f);
                    //thrusts.SetOverrideAccel("B", 20f);

                    if (VDistance > safe_base_height)
                    {
                ...
                OutStatusMode(0, 0, 0, 20f);
                return Complete;
            }
            public bool Dock()
            {
                bool Complete = false;
                float MaxUSpeed, MaxLSpeed, MaxFSpeed, FAccel = 0;
                thrusts.On();
                Vector3D MyPosCon = Vector3D.Transform(MyPos, DockMatrix);
                HDistance = (float)(new Vector3D(MyPosCon.GetDim(0), 0, MyPosCon.GetDim(2))).Length();
                VDistance = (float)(new Vector3D(0, MyPosCon.GetDim(1), 0)).Length();
                ...
                {
                    if (MyPosCon.GetDim(2) < 0)
                    {
                        gyrAng = GetForwNavAngles(ConnectorPoint1, DockMatrix);

                    }
                    //gyros.SetOverride(BlockNav, true, gyrAng * GyroMult, 1);
                }
                gyros.SetOverride(BlockNav, true, gyrAng * GyroMult, 1);
                MaxLSpeed = (float)Math.Sqrt(2 * Math.Abs(MyPosCon.GetDim(0)) * XMaxA) / 2; // (лево право)
                MaxUSpeed = (float)Math.Sqrt(2 * Math.Abs(MyPosCon.GetDim(2)) * YMaxA) / 2; // (вперед назад)
                MaxFSpeed = (float)Math.Sqrt(2 * VDistance * ZMaxA) / 3;                    // (вверх вниз)
               
                //--
                if (HDistance < 15) { MaxUSpeed = 0.6f; MaxLSpeed = 0.6f; }
                //if (Math.Abs(MyPosCon.GetDim(0)) < 1f && HDistance <= safe_base_distance) MaxLSpeed = 0.3f;
                //if (Math.Abs(MyPosCon.GetDim(2)) < 1f && HDistance <= safe_base_distance) MaxUSpeed = 0.3f;
                //if (HDistance > safe_base_distance) { MaxUSpeed = 0.1f; }
                if (LeftVelocityVector.Length() < MaxLSpeed && HDistance <= safe_base_distance) // 
                {
                    float LAccel = getAccel((float)(Math.Abs(Math.Max(MyPosCon.GetDim(0), MyPosCon.GetDim(2))) * AlignAccelMult), 0.3f);
                    thrusts.SetOverrideAccel("R", LAccel);
                }
                else
                {
                ...
                //--
                if ((UpVelocityVector.Length() < MaxUSpeed && Math.Abs(MyPosCon.GetDim(0)) < 1.0f && HDistance <= safe_base_distance) || 
                    (HDistance > safe_base_distance && UpVelocityVector.Length() < MaxUSpeed)) // 
                {
                    float UAccel = getAccel((float)(MyPosCon.GetDim(2) * AlignAccelMult), 0.3f);
                    if (HDistance > safe_base_distance && MyPosCon.GetDim(2) < 0)
                    {
                        UAccel *= -1;
                    }
                    thrusts.SetOverrideAccel("D", UAccel);
                }
                else
                {
                ...
                // --
                if (VDistance <= safe_base_height) MaxFSpeed = MaxFSpeed / 4;
                if (VDistance <= 15) MaxFSpeed = 0.3f;
                if (Math.Abs(MyPosCon.GetDim(1)) < 1f) MaxFSpeed = 0.1f;
                if ((VDistance > min_br_height) || ((Math.Abs(MyPosCon.GetDim(0)) < 1f) && (Math.Abs(MyPosCon.GetDim(2)) < 1f) && (ForwVelocityVector.Length() <= MaxFSpeed)))
                {
                    FAccel = getAccel((float)(MyPosCon.GetDim(1) * (Math.Abs(MyPosCon.GetDim(1)) < 1f ? 0.1f : AlignFAccelMult)), 0.1f);
                    thrusts.SetOverrideAccel("F", FAccel);
                }
                else
                {
                    thrusts.SetOverridePercent("F", 0);
                ...
                OutStatusMode(MaxFSpeed, MaxUSpeed, MaxLSpeed, FAccel);
                return Complete;
            }
            public bool ToBase()
            {
                ...
                MaxUSpeed = (float)Math.Sqrt(2 * Math.Abs(MyPosCon.GetDim(2)) * YMaxA) / 2;
                MaxFSpeed = (float)Math.Sqrt(2 * VDistance * ZMaxA) / 2;
                ...
                if (HDistance > 2.0f)
                {
                    ...
                    if (UpVelocityVector.Length() < MaxUSpeed)
                        thrusts.SetOverrideAccel("D", (float)(MyPosCon.GetDim(2) * AlignAccelMult));
                    else
                    {
                        thrusts.SetOverridePercent("D", 0);
                        thrusts.SetOverridePercent("U", 0);
                ...
                else
                {
                    // Выполним посадку
                    thrusts.ClearThrustOverridePersent();
                    if ((VDistance - DownBrDistance) < min_br_height)
                    {
                        // Тормозим
                        thrusts.On();
                        //thrusts.ClearThrustOverridePersent();
                    ...
                    {
                        if (ForwVelocityVector.Length() < 100)
                        {
                            thrusts.On();
                            thrusts.SetOverridePercent("F", 1.0f);
                        }
                        else
                        {
                            //thrusts.SetOverridePercent("D", 0f);
            ...
            public bool ToOrbit()
            {
                ...
                if (gravity)
                {
                    ...
                    thrusts.SetOverridePercent("U", 0);
                    thrusts.SetOverridePercent("D", 0);
                    if (ForwVelocityVector.Length() < 100.0f)
                        thrusts.SetOverridePercent("B", 1.0f);
                    else
                    {
                        thrusts.SetOverrideN("U", -ForwThrust);
                    }
                }
                else
                {
            ...
            //-------------------------------------------------
            public void OutStatusMode(float MaxFSpeed, float MaxUSpeed, float MaxLSpeed, float FAccel)
            {
                StringBuilder values = new StringBuilder();
                ...
                values.Append("------------------------------------------\n");
                values.Append("FAccel      : " + Math.Round(FAccel, 2).ToString() + "\n");
            ...
            public void Logic(string argument, UpdateType updateSource)
            {
                switch (argument)
                {
                    ...
                    case "to_orbit": { InitMode(mode.to_orbit); break; }
                ...
                if (updateSource == UpdateType.Update10)
                {
                    ...
                    if (curent_programm == programm.none)
                    {
                        ...
                        if (curent_mode == mode.to_orbit && !paused && ToOrbit())
                        {
                            curent_mode = mode.none; strg.SaveToStorage();
                        }
        ...
        public class MyStorage
        {
            ...
            public void LoadFromStorage()
            {
                ...
                if (nav.curent_mode != Nav.mode.none)
                {
                ...
                //if (nav.curent_mode == Nav.mode.dock || nav.curent_mode == Nav.mode.un_dock || nav.curent_mode == Nav.mode.none)
                //{
                //    nav.InitBlock(connector.obj);
                //}
                //else
