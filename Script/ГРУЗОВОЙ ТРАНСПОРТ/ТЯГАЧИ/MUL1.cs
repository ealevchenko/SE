...
namespace MUL1
{
    public sealed class Program : MyGridProgram
    {
        ...
        // Название 
        string NameGroup = "MUL1";
        string NameCockpit = "MUL1-Кресло пилота [LCD]";
        string NameConnector = "CARGO1-Коннектор парковка";
        string NameLCD1 = "MUL1-ЖК-панель info";
        ...
        bool enable_backward = false; // Бит держать горизонт
        //int clock = 0;
        ...
        public void Main(string argument, UpdateType updateSource)
        {
            ...
            if (updateSource == UpdateType.Update10)
            {
                ...
                // Держать зад к центру
                if (!connector.Connected && enable_backward)
                {
                    KeepBackward();
                }
            ...
            lcd1.Write("ПОЛЕТ:" + (enable_horizont ? "Горизонт" : (enable_backward ? "Жопа вниз": "")) + "\n", true);
        ...
        {
            giros.SetGyro(cocpit.GetAxisBackward());
        }
        ...
        public class Cockpit
        {
            ...
            public Vector3D GetAxisBackward()
            {
                Vector3D grav = Vector3D.Normalize(cockpit.GetNaturalGravity());
                Vector3D axis = grav.Cross(cockpit.WorldMatrix.Backward);
                if (grav.Dot(cockpit.WorldMatrix.Backward) < 0)
                {
                    axis = Vector3D.Normalize(axis);
                }
                return axis;
