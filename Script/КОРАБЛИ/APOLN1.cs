...
namespace APOLN1
{
    public sealed class Program : MyGridProgram
    {
        ...
        // Название 
        string NameGroup = "APOLN1";
        string NameCockpit = "APOLN1-Кокпит истребителя";
        string NameConnector = "APOLN1-Коннектор";
        string NameLCD1 = "APOLN1-ЖК-панель 1";
        ...
        public Program()
        {
            ...
            gears = new LandingGear(NameGroup);
        ...
        public void Main(string argument, UpdateType updateSource)
        {
            ...
            lcd1.Write(gears.GetLockModeOfText() + "\n", true);
            lcd1.Write(batterys.GetModeOfText() + "\n", true);
            lcd1.Write("ПОЛЕТ:" + (enable_horizont ? "Горизонт" : "") + "\n", true);
        ...
        {
            List<IMyLandingGear> gearlist = new List<IMyLandingGear>();
            public int Count { get { return gearlist.Count(); } }
            public LandingGear(string name_group)
            {
                _scr.GridTerminalSystem.GetBlocksOfType<IMyLandingGear>(gearlist, r => r.CustomName.Contains(name_group));
                _scr.Echo("gearlist:[" + name_group + "]" + ((gearlist != null && gearlist.Count() > 0) ? ("Ок") : ("not gearlist")));
            }
            public string GetLockMode()
            {
                string result = "";
                foreach (IMyLandingGear batt in gearlist)
                {
                    result = batt.LockMode.ToString();
                }
                return result;
            }
            public string GetLockModeOfText()
            {
                string result = "ШАССИ:";
                foreach (IMyLandingGear gear in gearlist)
                {
