...
namespace KROTIK_A1P
{
    public sealed class Program : MyGridProgram
    {
        ...
        // Дрель
        public class ShipDrill
        {
            List<IMyShipDrill> drilllist = new List<IMyShipDrill>();
            public int Count { get { return drilllist.Count(); } }
            public ShipDrill(string name_group)
            {

                _scr.GridTerminalSystem.GetBlocksOfType<IMyShipDrill>(drilllist, r => r.CustomName.Contains(name_group));
                _scr.Echo("drilllist: " + ((drilllist != null && drilllist.Count() > 0) ? ("Ок") : ("not drilllist")));
            }

            public void On()
            {
                foreach (IMyShipDrill drill in drilllist)
                {
