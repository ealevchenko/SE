...
/// </summary>
namespace STEP_DRILL
{
    public sealed class Program : MyGridProgram
    {
        string NameObj = "[StepDrill]";
        ...
        public Program()
        {
            ...
            lcd_storage1 = new LCD(NameObj + "-LCD [storage1]");
            lcd_debug = new LCD(NameObj + "-LCD-DEBUG");
            lcd_work1 = new LCD(NameObj + "-LCD-Work 1"); // [StepDrill]-LCD-Work 1
            lcd_status = new LCD(NameObj + "-LCD-status");
            bats = new Batterys(NameObj);
            connector_forw = new Connector(NameObj + "-Коннектор [start]");
            connector_back = new Connector(NameObj + "-Коннектор [end]");
            //mergeblock_forw = new ShipMergeBlock(NameObj + "-Соединитель forw");
            //mergeblock_forw.Off();
            mergeblock_back = new ShipMergeBlock(NameObj + "-Соединитель [end]");
        ...
        public class Upr
        {
            ...
            public void StartDrill()
            {
                if (curent_mode == mode.none)
                {
                    ...
                    if (pistones.Position > 18.0f && connector_forw.Connected)
                    {
            ...
            public bool DrillForward()
            {
                ...
                if (pistones.Position <= 18.0f)
                {
                    ...
                    if (pistones.Position > 1.0f) // && connector_forw.Unconnected
                    {
                        welders.On();
                    }
                }
